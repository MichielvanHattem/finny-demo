...
if check_password():
    ...
    # CHAT
        ...
        # Neutrale sessie-disclaimer + uitklapbare disclaimer
        st.info("Finny kan fouten maken. Controleer belangrijke informatie altijd zelf.")
        with st.expander("Disclaimer"):
            st.markdown(
                "Finny is een AI-assistent. Antwoorden zijn gebaseerd op de beschikbare gegevens "
                "en dienen niet als professioneel advies. Raadpleeg bij twijfel altijd uw accountant "
                "of een andere professionele adviseur."
            )
        ...
        # Sessieniveau feedback (één keer per gesprek)
        has_assistant_msg = any(
            m.get("role") == "assistant" for m in st.session_state.messages
        )
        if has_assistant_msg and not st.session_state.session_feedback_given:
            st.markdown("---")
            st.subheader("Heeft dit gesprek je geholpen?")
            ...
            if good:
                ...
                st.success("Dank je voor je feedback!")
            ...
            if (
                ...
                if st.button("Verstuur feedback", key="conv_fb_send"):
                    ...
                    st.session_state.session_feedback_given = True
                    st.session_state.show_conv_feedback_input = False
                    st.success("Dank je, je feedback is verstuurd.")
