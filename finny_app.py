...
# ==========================================
# 1B. LEGAL GATEWAY (DISCLAIMER)
# ==========================================

def show_legal_gateway():
    st.title("Welkom bij Finny")
    st.markdown("### Eerst even dit")
    
    st.info(
        "Finny is een AI-assistent. Antwoorden kunnen fouten bevatten en zijn **geen** vervanging voor "
        "jouw accountant. Gebruik is op eigen risico."
    )
    
    with st.expander("Lees de volledige juridische voorwaarden"):
        st.markdown(
            """
            **1. Geen professioneel advies:** Finny is een hulpmiddel. Raadpleeg altijd een expert voor beslissingen.
            **2. Aansprakelijkheid:** Buddy Workforce BV aanvaardt geen aansprakelijkheid voor schade door gebruik.
            **3. Privacy:** U kiest zelf of wij uw profiel opbouwen voor betere antwoorden.
            """
        )

    col1, col2 = st.columns([0.05, 0.95])
    with col1:
        agreed = st.checkbox("", label_visibility="collapsed")
    with col2:
        st.markdown("Ik ga akkoord met de voorwaarden.")

    st.subheader("Personaliseren?")
    consent_profile = st.toggle("Ja, leer van mijn vragen voor betere antwoorden (Klantprofiel)", value=False)

    if st.button("Start Gesprek", disabled=not agreed, type="primary"):
...
def render_message_extras(content: str, idx: int):
    """
    Toont alleen ESSENTIËLE interactie elementen.
    Geen duimpjes meer per bericht!
    """
    # 1. De minimalistische disclaimer (zoals Gemini)
    st.caption("🤖 _AI-gegenereerd antwoord. Controleer feiten. Geen prof. advies._")

    # 2. Escalatie (zakelijk noodzakelijk)
...
if check_password():
    ...
    # --- SIDEBAR (Rustig menu & Feedback) ---
    with st.sidebar:
        st.image(main_logo, width=120)
        st.markdown("---")
        
        # Menu Navigatie
        menu = {"chat": "💬 Chat", "history": "📂 Geschiedenis", "share": "📩 Accountant"}
        sel = st.radio("Menu", list(menu.keys()), format_func=lambda x: menu[x])
    ...
    if st.session_state.current_view == "chat":
        ...
        for idx, m in enumerate(st.session_state.messages):
            with st.chat_message(m["role"]):
                st.write(m["content"])
                if m["role"] == "assistant":
                    render_message_extras(m["content"], idx)
        ...
        if prompt:
            ...
            with st.chat_message("assistant"):
                with st.spinner("Rekenmachine loopt..."):
                    # Check follow-up memory
                    pending = st.session_state.get("pending_followup")
                    question = prompt
                    
                    # Logic voor Winst-Trend follow-up
                    ...
                    # Prompting
                    tone = "Jip-en-Janneke" if st.session_state.client_profile.get("finance_knowledge", 2) < 3 else "Zakelijk"
                    
                    # Trend-trigger (zet geheugen klaar)
                    ...
                    sys_prompt = (
                        ...
                        f"Tone: {tone}. Antwoord kort. Geen verzinsels.\n"
                    ...
                    render_message_extras(reply, len(st.session_state.messages)-1)
    ...
    # --- VIEW: SHARE ---
    elif st.session_state.current_view == "share":
