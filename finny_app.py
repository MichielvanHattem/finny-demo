...
def load_data():
    ...
    def clean_code(val): return str(val).split('.')[0].strip()
    ...
    if os.path.exists("Finny_Transactions.csv"):
        try:
            ...
            if 'Finny_Year' in df.columns:
                ...
                if not valid_years.empty: data["latest_year"] = int(valid_years.max())
    ...
    for txt in txt_files:
        try:
            with open(txt, "r", encoding="utf-8") as f: data["company_context"] += f"\n--- PROFIEL ({txt}) ---\n{f.read()}\n"
        except: 
            try:
                with open(txt, "r", encoding="latin1") as f: data["company_context"] += f"\n--- PROFIEL ({txt}) ---\n{f.read()}\n"
            except: pass
...
def get_intent(client, question, data):
    ...
    try:
        res = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "Je bent een router. Kies 'PDF' voor algemene vragen/trends. Kies 'CSV' voor specifieke transactievragen. Output JSON: {source: 'PDF'|'CSV', years: [], keywords: []}"},
...
def analyze_csv_costs(data, intent):
    if data["trans"] is None: return "Geen transacties beschikbaar."
    df = data["trans"].copy()
    ...
    if 'Description' in df_total.columns:
        ...
        top = df_total.groupby('Description')['AmountDC_num'].sum().sort_values(ascending=False).head(5).reset_index()
...
if check_password():
    ...
    # VIEW: CHAT
        ...
        if prompt := st.chat_input("Vraag Finny..."):
            ...
            with st.chat_message("assistant", avatar=finny_av):
                with st.spinner("..."):
                    ...
                    tone_instruction = "Gebruik Jip-en-Janneke taal. Vermijd jargon zoals 'immateriële activa', zeg gewoon 'bezittingen'."
                    if fin_know >= 4:
                    ...
                    for m in st.session_state.messages[-3:]: msgs.append({"role": "user" if m["role"]=="user" else "assistant", "content": m["content"]})
        ...
        st.title("Accountant")
        if not st.session_state.conversations: st.info("Geen gesprekken.")
        else:
            with st.form("sh"):
                sel = {}
                for i, c in enumerate(st.session_state.conversations):
                    sel[i] = st.checkbox(f"{c['timestamp'].strftime('%H:%M')} - {c['title']}", value=c.get("shared", False), key=i)
                if st.form_submit_button("Opslaan"):
                    for i, v in sel.items(): st.session_state.conversations[i]['shared'] = v
