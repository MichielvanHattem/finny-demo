...
FALLBACK_SYNONYMS = {
    ...
    "omzet": ["omzet", "opbrengst", "verkopen", "sales", "revenue", "factuur", "omzetten", "opbrengsten"],
}
...
def load_data():
    ...
    if os.path.exists("Finny_Transactions.csv"):
        try:
            ...
            # Datum parsing voor maand-functionaliteit
            ...
            for col in ["Description", "AccountName", "Finny_GLDescription", "Finny_GLCode", "AmountDC_num", "Date"]:
...
def classify_intent(client: OpenAI, question: str) -> dict:
    ...
    system = (
        ...
        "- SPECIFIC_COST (specifieke post of omzet)\n"
...
def build_csv_query(data: dict, intent: dict, years: list[int], is_monthly: bool = False):
    df = data.get("trans")
    ...
    # Zoeken op term (kosten of omzet)
    if intent["type"] in {"SPECIFIC_COST", "DETAILS", "TREND"} and term:
    ...
    lines = [f"Totaal geselecteerd ({' '.join(str_years)}): € {total:,.2f}"]
    
    # --- NIEUWE MAAND FUNCTIONALITEIT ---
    if is_monthly and "Date_Parsed" in df_curr.columns:
        # Groepeer per maand
        monthly = df_curr.groupby(df_curr["Date_Parsed"].dt.to_period("M"))["AmountDC_num"].sum()
        lines.append("\n**Verloop per maand:**")
        lines.append("| Maand | Bedrag |")
        lines.append("|---|---|")
        for period, amount in monthly.items():
            lines.append(f"| {period} | € {amount:,.2f} |")
            
    # Drill down (Details)
    elif intent["type"] == "DETAILS" or (intent["type"] == "SPECIFIC_COST" and not df_curr.empty):
        top = df_curr.assign(Abs=df_curr["AmountDC_num"].abs()).sort_values("Abs", ascending=False).head(5)
        lines.append("\n**Top 5 Transacties:**")
        for _, row in top.iterrows():
            desc = row.get('Description', 'Onbekend')
            amt = row.get('AmountDC_num', 0)
            date = row.get('Date', '')
...
def build_analysis(client, question, data):
    ...
    # Check of de gebruiker "per maand" wil zien
    is_monthly = "maand" in question.lower()
    
    if intent["type"] == "CHAT" and not is_monthly:
        return {"context": data.get("pdf_text"), "source": "PDF", "intent": intent, "years": years}
    
    csv_txt, total = build_csv_query(data, intent, years, is_monthly)
    if csv_txt:
        return {"context": csv_txt, "source": "CSV", "intent": intent, "years": years}
    
    # Fallback als CSV niks vindt
    return {"context": data.get("pdf_text"), "source": "PDF", "intent": intent, "years": years}
...
if check_password():
    ...
    with st.sidebar:
        ...
        menu = {"intro": "👋 Kennismaking", "chat": "💬 Chat", "history": "📂 Geschiedenis", "share": "📩 Accountant"}
        opts = list(menu.keys())
